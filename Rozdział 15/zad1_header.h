...
class Tv
{
	...
	public:
		...
		void Set_Tryb(Remote & _r) { _r.Set_Tryb(); }	//dodane, ustawianie trybu
};
...
class Remote
{
	private:
		...
		int n_tryb;		//dodany tryb zwykly lub interaktywny
	public:
		friend class Tv;
		enum { Zwykly, Interaktywny };
		Remote(int _m = Tv::TV, int _t = Zwykly) : n_mode(_m), n_tryb(_t) {}
		...
		void Show_Tryb(); 	//dodane wyswietlanie trybu pracy pilota
		void Set_Tryb() { n_tryb = (n_tryb == Zwykly) ? Interaktywny : Zwykly; }	//dodane, ustawianie trybu
};
